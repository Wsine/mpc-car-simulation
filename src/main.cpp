    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (!s.empty()) {
        ...
        if (event == "telemetry") {
          ...
          double epsi = atan(coeffs[1]); // atan(polyeval_deriv(coeffs, 0))
          ...
          double steer_value = -control_values[0];
          double throttle_value = control_values[1];
          ...
          std::cout << "control: steer = " << steer_value
          ...
          std::vector<double> next_x_vals(10);
          std::vector<double> next_y_vals(10);
          for (int i = 0; i < 10; i++) {
            next_x_vals[i] = 4 * i;
